  ...
  return (
    <div className="w-full py-6">
      <div className="flex items-center justify-between max-w-sm mx-auto relative">
        ...
        {steps.map((step, index) => {
          ...
          return (
            <div key={step.num} className="relative z-10 flex flex-col items-center">
              ...
              <span 
                className={`mt-2 text-xs font-medium absolute -bottom-6 whitespace-nowrap transition-colors duration-300
                  ${isCurrent || isCompleted ? 'text-pipa-orange' : 'text-slate-400'}
                `}
              >
                {step.label}
              </span>
        ...
        {/* Progress Bar Fill */}
        <div 
          className="absolute left-0 top-1/2 -translate-y-1/2 h-1 bg-pipa-orange rounded-full z-0 transition-all duration-500 ease-out"
